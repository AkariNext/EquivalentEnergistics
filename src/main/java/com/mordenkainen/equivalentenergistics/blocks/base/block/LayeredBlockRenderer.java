...
public class LayeredBlockRenderer implements ISimpleBlockRenderingHandler { // NOPMD
    ...
    public boolean renderWorldBlock(final IBlockAccess world, final int x, final int y, final int z, final Block block,
        ...
        return Minecraft.isAmbientOcclusionEnabled() && block.getLightValue() == 0
                ? renderLayeredBlockWithAmbientOcclusion(renderer, block, meta, x, y, z, r, g, b)
                : renderLayeredBlockWithColorMultiplier(renderer, block, meta, x, y, z, r, g, b);
    }
    ...
    private boolean renderLayeredBlockWithAmbientOcclusion(final RenderBlocks renderer, final Block block,
            final int metadata, final int x, final int y, final int z, final float r, final float g, final float b) { // NOPMD
        ...
        final boolean boundryTest[] = { renderer.renderMinY <= 0.0D, renderer.renderMaxY >= 1.0D,
                renderer.renderMinZ <= 0.0D, renderer.renderMaxZ >= 1.0D, renderer.renderMinX <= 0.0D,
        ...
        for (final ForgeDirection face : ForgeDirection.VALID_DIRECTIONS) {
            if (renderer.renderAllFaces || block.shouldSideBeRendered(
                ...
                final ForgeDirection sides[] = EnumSet
                        .complementOf(EnumSet.of(face, face.getOpposite(), ForgeDirection.UNKNOWN))
                        .toArray(new ForgeDirection[0]);
                for (int i = 0; i < 4; i++) {
                ...
                if (transparencies[0] || transparencies[2]) {
                    brightnesses[4] = block.getMixedBrightnessForBlock(
                            renderer.blockAccess,
                            x + corners[face.ordinal()][0][0],
                            y + corners[face.ordinal()][0][1],
                            z + corners[face.ordinal()][0][2]);
                    occlusionValues[4] = renderer.blockAccess.getBlock(
                            x + corners[face.ordinal()][0][0],
                            y + corners[face.ordinal()][0][1],
                            z + corners[face.ordinal()][0][2]).getAmbientOcclusionLightValue();
                } else {
                ...
                if (transparencies[1] || transparencies[2]) {
                    brightnesses[5] = block.getMixedBrightnessForBlock(
                            renderer.blockAccess,
                            x + corners[face.ordinal()][1][0],
                            y + corners[face.ordinal()][1][1],
                            z + corners[face.ordinal()][1][2]);
                    occlusionValues[5] = renderer.blockAccess.getBlock(
                            x + corners[face.ordinal()][1][0],
                            y + corners[face.ordinal()][1][1],
                            z + corners[face.ordinal()][1][2]).getAmbientOcclusionLightValue();
                } else {
                ...
                if (transparencies[1] || transparencies[3]) {
                    brightnesses[6] = block.getMixedBrightnessForBlock(
                            renderer.blockAccess,
                            x + corners[face.ordinal()][2][0],
                            y + corners[face.ordinal()][2][1],
                            z + corners[face.ordinal()][2][2]);
                    occlusionValues[6] = renderer.blockAccess.getBlock(
                            x + corners[face.ordinal()][2][0],
                            y + corners[face.ordinal()][2][1],
                            z + corners[face.ordinal()][2][2]).getAmbientOcclusionLightValue();
                } else {
                ...
                if (transparencies[0] || transparencies[3]) {
                    brightnesses[7] = block.getMixedBrightnessForBlock(
                            renderer.blockAccess,
                            x + corners[face.ordinal()][3][0],
                            y + corners[face.ordinal()][3][1],
                            z + corners[face.ordinal()][3][2]);
                    occlusionValues[7] = renderer.blockAccess.getBlock(
                            x + corners[face.ordinal()][3][0],
                            y + corners[face.ordinal()][3][1],
                            z + corners[face.ordinal()][3][2]).getAmbientOcclusionLightValue();
                } else {
                ...
                if (boundryTest[face.ordinal()] || !renderer.blockAccess
                        .getBlock(x + face.offsetX, y + face.offsetY, z + face.offsetZ).isOpaqueCube()) {
                    faceBrightness = block.getMixedBrightnessForBlock(
                            renderer.blockAccess,
                            x + face.offsetX,
                            y + face.offsetY,
                            z + face.offsetZ);
                ...
                final float faceocclusion = renderer.blockAccess
                        .getBlock(x + face.offsetX, y + face.offsetY, z + face.offsetZ).getAmbientOcclusionLightValue();

                final float lightValueTL = (faceocclusion + occlusionValues[0]
                        + occlusionValues[7]
                        + occlusionValues[3]) / 4.0F;
                final float lightValueBL = (faceocclusion + occlusionValues[0]
                        + occlusionValues[4]
                        + occlusionValues[2]) / 4.0F;
                final float lightValueBR = (faceocclusion + occlusionValues[1]
                        + occlusionValues[6]
                        + occlusionValues[3]) / 4.0F;
                final float lightValueTR = (faceocclusion + occlusionValues[1]
                        + occlusionValues[5]
                        + occlusionValues[2]) / 4.0F;

                renderer.brightnessTopLeft = renderer
                        .getAoBrightness(brightnesses[0], brightnesses[7], brightnesses[3], faceBrightness);
                renderer.brightnessBottomLeft = renderer
                        .getAoBrightness(brightnesses[0], brightnesses[4], brightnesses[2], faceBrightness);
                renderer.brightnessBottomRight = renderer
                        .getAoBrightness(brightnesses[1], brightnesses[6], brightnesses[3], faceBrightness);
                renderer.brightnessTopRight = renderer
                        .getAoBrightness(brightnesses[1], brightnesses[5], brightnesses[2], faceBrightness);
    ...
    private void setRendererColors(final RenderBlocks renderer, final float r, final float g, final float b,
            final float sideMult, final float multTopLeft, final float multBotLeft, final float multBotRight,
            final float multTopRight, final boolean flag) {
        if (flag) {
    ...
    private boolean renderLayeredBlockWithColorMultiplier(final RenderBlocks renderer, final Block block,
            final int metadata, final int x, final int y, final int z, final float r, final float g, final float b) {
        renderer.enableAO = false;
        final Tessellator tessellator = Tessellator.instance;
        boolean flag = false;
        ...
        final boolean[] blockTests = { renderer.renderMinY > 0.0D, renderer.renderMaxY < 1.0D,
                renderer.renderMinZ > 0.0D, renderer.renderMaxZ < 1.0D, renderer.renderMinX > 0.0D,
                renderer.renderMaxX < 1.0D };
    ...
    private void renderFaceWithLayers(final RenderBlocks renderer, final Block block, final int metadata,
            final ForgeDirection dir, final double x, final double y, final double z, final boolean inv) {
        ...
        renderFaceByDir(
                ...
                y,
                z,
                renderer.getBlockIconFromSideAndMetadata(block, dir.ordinal(), metadata));
        final int numLayers = inv ? layers.numLayers(block, metadata)
                : layers.numLayers(renderer.blockAccess, block, (int) x, (int) y, (int) z, metadata);
        for (int i = 1; i <= numLayers; i++) {
            final IIcon layer = inv ? layers.getLayer(block, dir.ordinal(), metadata, i)
                    : layers.getLayer(
                            renderer.blockAccess,
                            block,
                            (int) x,
            ...
            if (layer != null) {
                renderFaceByDir(
                        ...
                        block,
                        dir,
                        x + ((dir.offsetX * i) * 0.00001D),
                        y + ((dir.offsetY * i) * 0.00001D),
                        z + ((dir.offsetZ * i) * 0.00001D),
                        layer);
            }
        }
    }

    private void renderFaceByDir(final RenderBlocks renderer, final Block block, final ForgeDirection dir,
            final double x, final double y, final double z, final IIcon icon) {
        switch (dir) {
            case DOWN:
