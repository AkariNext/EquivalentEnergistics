    ...
    public List<String> getWailaBody(final ItemStack itemStack, final List<String> currenttip,
        ...
        if (tag.hasKey(TAG_NAME)) {
            ...
            if (innerTag.hasKey(EMC_TAG)) {
                currenttip.add(
                        StatCollector.translateToLocal("tooltip.emc.name") + " "
                                + SpecialChars.RED
                                + CommonUtils.formatEMC(innerTag.getDouble(EMC_TAG))
                                + SpecialChars.RESET);
            }
        }
        ...
        currenttip.add(
                StatCollector.translateToLocal("tooltip.status.name") + " "
                        + (state == CondenserState.ACTIVE || state == CondenserState.IDLE ? SpecialChars.GREEN
                                : SpecialChars.RED)
                        + state.getStateName()
                        + SpecialChars.RESET);
